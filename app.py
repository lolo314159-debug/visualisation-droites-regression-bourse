...
# --- 3. FILTRAGE DYNAMIQUE SUR HISTORIQUE LONG ---
@st.cache_data(ttl=3600)
def get_filtered_list(stocks_dict, r2_threshold):
    # Pour Air Liquide et les autres, on télécharge l'historique long pour un filtre précis
    tickers = list(stocks_dict.values())
    # On limite à 10 ans pour le filtre pour garder de la performance
...
r2_min = st.sidebar.slider("2. R² minimum (Qualité)", 0.0, 1.0, 0.90, 0.01)

base_stocks, bench_ticker = get_index_components(idx_choice)

with st.sidebar:
    st.write("---")
    with st.spinner("Calcul des R² historiques..."):
    ...
    if filtered_names:
        ...
        st.error("Aucune action ne correspond.")
        st.stop()

# --- 5. ANALYSE ET GRAPHIQUE ---
if symbol:
    # Analyse finale sur l'historique max
    ...
    c1.metric("Fiabilité (R²)", f"{r2:.4f}")
    c2.metric("Volatilité Ann.", f"{vol:.2f} %")
    c3.metric("Prix Actuel", f"{curr:.2f} €")
    c4.metric("Position / Droite", f"{((curr/theo)-1)*100:+.2f}%")
    ...
    t2.metric("Support -1σ", f"{np.exp(y_pred_log[-1] - std_dev):.2f}")
    t3.metric("Théorique", f"{theo:.2f}")
    t4.metric("Résistance +1σ", f"{np.exp(y_pred_log[-1] + std_dev):.2f}")
    ...
    fig.add_trace(go.Scatter(x=df_full.index, y=np.exp(y_pred_log), name="Régression", line=dict(color='gold', dash='dash')))
