...
def get_r2_interpretation(v):
    if v > 0.98: return "💎 **Qualité Diamant** : La croissance est d'une régularité absolue, presque mathématique."
    if v > 0.93: return "🌟 **Qualité Exceptionnelle** : Tendance extrêmement solide avec très peu de déviations historiques."
    if v > 0.85: return "✅ **Très Bonne Qualité** : Une action de croissance robuste malgré quelques cycles passagers."
    if v > 0.70: return "⚠️ **Qualité Correcte** : Tendance visible mais perturbée par une volatilité ou une cyclicité marquée."
    return "❌ **Qualité Faible/Spéculative** : Le cours ne suit pas une trajectoire log-linéaire stable."

def get_trading_signal(curr, s1_u, s1_d, s2_u, s2_d):
    if curr <= s2_d: return "🔵 **ACHAT FORT** (Zone -2σ : Rare opportunité statistique)", "blue"
    if curr <= s1_d: return "🟢 **ACHAT** (Zone -1σ : Support historique)", "green"
    if curr >= s2_u: return "🔴 **VENTE FORTE** (Zone +2σ : Surévaluation majeure)", "red"
    if curr >= s1_u: return "🟠 **ALLÉGEMENT** (Zone +1σ : Tension haussière)", "orange"
    return "⚪ **NEUTRE** (Prix proche de sa trajectoire moyenne)", "gray"
...
signal_text, _ = get_trading_signal(curr, s1_u, s1_d, s2_u, s2_d)

# AFFICHAGE DES RÉSULTATS
st.markdown(f"### 📈 {stock_name} ({symbol})")

# Bloc d'interprétation R2 et Signal (Compact)
col_info_1, col_info_2 = st.columns([2, 1])
with col_info_1:
    st.info(f"**Analyse de fiabilité (R² = {r2:.4f})** : {get_r2_interpretation(r2)}")
with col_info_2:
    st.warning(f"**Position actuelle** : {signal_text}")
...
m4.metric("Position / Droite", f"{((curr/theo)-1)*100:+.1f}%")
m5.metric("Score R²", f"{r2:.4f}")
...
def create_plot(is_log):
    ...
    fig.add_trace(go.Scatter(x=dates, y=df_full.values, name="Prix réel", line=dict(color='#00D4FF', width=1.5)))
    fig.add_trace(go.Scatter(x=dates, y=y_trend, name="Droite théorique", line=dict(color='gold', width=1, dash='dash')))
...
t[2].caption("PRIX THÉORIQUE"); t[2].write(f"**{theo:.2f}**")
